...
# Deal with \N
# Changed by Le: adding "+pd.to_timedelta('1 days')" to make expired date later than due date
train_df['repay_date'] = train_df[['due_date', 'repay_date']].apply(
    lambda x: x['repay_date'] if x['repay_date'] != '\\N' else x['due_date']+pd.to_timedelta('1 days'), axis=1
)
train_df['repay_amt'] = train_df['repay_amt'].apply(lambda x: x if x != '\\N' else 0).astype('float32')

# set ('repay_date'-'auditing_date') to be label
...
for f in ['listing_id', 'order_id', 'due_date', 'repay_date', 'repay_amt']:
    del repay_log_df[f]
# 'repay_mean': average repaying possibility for each user_id
...
# max,median,mean,std of 'early_repay_days' for each user_id
repay_log_df = repay_log_df.merge(
...
# max,median,mean,std of 'due_amt' for each user_id
repay_log_df = repay_log_df.merge(
    group['due_amt'].agg({
